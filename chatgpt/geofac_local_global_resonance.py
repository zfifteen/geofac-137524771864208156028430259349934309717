...
Local-global resonance factorization prototype using Dirichlet kernels.
Generates candidates near sqrt(N), applies p-adic filtering, ranks by
resonance score, and validates via arithmetic predicate (N mod d == 0).

This script is intentionally minimal and geometry-first: it only ever
reports factors found by ranking a bounded candidate set and checking
N % d == 0 on the top-ranked entries.
"""
import argparse
import math
import random
...
def adaptive_precision(N: int) -> int:
    """
    Adaptive precision heuristic based on N's bit length.
...
def small_primes(limit: int = 97) -> List[int]:
    """Generate primes up to limit for p-adic filtering."""
    primes: List[int] = []
...
def effective_window(N: int, base_window: int) -> int:
    """
    Scale the search window with N so very large semiprimes (e.g. 127-bit)
    are not artificially excluded.

    For the 127-bit challenge, the true factors sit about 11–12% away
    from sqrt(N); we therefore enforce a minimum window of roughly
    sqrt(N) / 8.
    """
    root = math.isqrt(N)
    # Preserve caller-provided window for smaller N; enlarge only when needed.
    scaled = root // 8
    if scaled <= base_window:
        return base_window
    return scaled
...
def geofac_local_global(
    ...
    Args:
        N: Integer to factor (intended to be a semiprime).
        window: Base half-width of search window around sqrt(N).
    ...
    # Scale the window so the 127-bit challenge isn’t trivially excluded.
    eff_window = effective_window(N, window)

    candidates = generate_candidates(N, eff_window, samples, seed, N_mod)
...
def main() -> None:
    ...
    ap.add_argument("N", type=int, help="Integer to factor (target semiprime).")
